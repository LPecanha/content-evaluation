 ...
 * e.g. "phish-of-the-week" → "Phish Of The Week"
 *
 * @param string $id
 * @return string
 */
...
<div class="wrap cv-admin-report">
	...
	<h1 class="wp-heading-inline"><?php esc_html_e( 'Content Vote Report', 'content-vote' ); ?></h1>
	...
	<div class="cv-summary-bar">
		<span>
			...
			printf(
				...
				esc_html( _n( '%d result', '%d results', $total_rows, 'content-vote' ) ),
				(int) $total_rows
			);
			?>
		</span>
	...
	<?php else : ?>
		...
		<?php // Build a URL → title lookup from pages_with_titles for fast access in the loop. ?>
		<?php
		$title_map = array();
		...
		<table class="widefat fixed striped cv-report-table">
			<thead>
				<tr>
					...
					<th class="cv-col-section">
						<?php echo cv_sort_link( 'section_id', __( 'Section', 'content-vote' ), $current_orderby, $current_order ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
					</th>
					<th class="cv-col-up">
			...
			<tbody>
				<?php foreach ( $rows as $row ) : ?>
					<?php
					$up        = (int) $row['total_up'];
					$down      = (int) $row['total_down'];
					$total     = (int) $row['total_votes'];
					$score_pct = $total > 0 ? round( ( $up / $total ) * 100 ) : 0;
					$up_pct    = $score_pct;

					// Resolve page title from the lookup map.
					$page_title = $title_map[ $row['page_url'] ] ?? $row['page_url'];
					?>
					<tr>
						...
						<td class="cv-col-section">
							<?php echo esc_html( cv_humanize_section_id( $row['section_id'] ) ); ?>
							<span class="cv-section-raw"><?php echo esc_html( $row['section_id'] ); ?></span>
						</td>
						...
						<td class="cv-col-bar">
							<div class="cv-bar" title="<?php echo (int) $up_pct; ?>% positive">
								<div class="cv-bar__fill cv-bar__fill--up" style="width:<?php echo (int) $up_pct; ?>%"></div>
							</div>
						</td>
						<td class="cv-col-date">
							<?php echo esc_html( wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $row['last_vote'] ) ) ); ?>
						</td>
					</tr>
				<?php endforeach; ?>
			</tbody>
